...
- Press **Ctrl+P** (**Cmd+P** on a Mac) anywhere in Counselor, or use the new **Find** button on the toolbar, and a search box opens. Type any part of a name, a **hex coordinate**, or an **order**, and press **Enter**: Counselor switches to the right tab, selects the row, and centres the map on its hex.
...
- It also searches **this turn's results**, exactly as each tab reports them - a character's orders, skills and where they went; a city's production; a hex's full report; a nation's rumours and encounters. Something happened and you cannot remember who it happened to? Type a word from the report - `ambush`, `conjure`, the name of a ship - and Counselor lists everyone whose results mention it.
- Several words narrow rather than widen: `targaryen 1122` finds only the Targaryens on that hex.
- The best matches come first: an exact hex, then names, then everything else - so the thing you meant is at the top instead of buried.
...
### Sorting a table no longer closes the client
- Clicking a column header to sort could **crash Counselor** while the map was still setting itself up. Fixed.

### Website (clashlegends.com)
- **Game instructions sit beside the sign-up form.** On the **Sign Up** page a game's instructions now appear in their own column next to the nation picker instead of stretched across the top, so you can read what makes the game special while ordering your preferences. On a phone they stack, instructions first.
...
### Website (clashlegends.com)
- **My Games loads quickly again** - it was getting slow, and could fail to load entirely, for players with a long match history.
- **Every game you have ever played** now appears under **Past games**, not just the ones that made a leaderboard. Older campaigns and short games that used to be missing are back, each with its nation and final turn.
