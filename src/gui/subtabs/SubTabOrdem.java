...
public class SubTabOrdem extends TabBase implements IPopupTabGui, Serializable {
    ...
    /**
     * Replace the Matisse-generated raster button icons with crisp, theme-aware
     * SVG icons (16px to fit these compact order buttons), matching the
     * main-toolbar treatment so they read well in dark mode. Tooltips/sizes set
     * by the .form are kept.
     */
    ...
    private void initComponents() {
        ...
        jbRepeat.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/submit.png"))); // NOI18N
        jbRepeat.setToolTipText(labels.getString("REPEAT.AJUDA.TOOLTIP")); // NOI18N
        jbRepeat.setBorder(javax.swing.BorderFactory.createEmptyBorder(3, 3, 3, 3));
        jbRepeat.setFocusable(false);
...
//        jtListaOrdens.setFont(new Font(font.getName(), Font.PLAIN, font.getSize() * 2));
    ...
    /**
     * Newbie safeguard: after an action is saved, uncheck the "ALL" box so the
     * per-order checks re-engage for the next action. Power users can opt out
     * by adding MaintainOrdersAllChecked=1 to properties.config (no Settings
     * UI), which keeps ALL checked until they clear it themselves.
     */
    ...
    /**
     * The orders selectable for a given slot, built exactly as the order combo
     * is (same ALL-checkbox state), so a caller outside the panel judges
     * availability the same way the player sees it.
     */
...
//                log.fatal("muda foco aki");
    ...
    /**
     * KI-017: true when the order SAVED in this slot declares the same
     * parameter control as the one being built, so its value is safe to restore
     * into the widget. "Variado" is pass-through: a spell order records the
     * placeholder rather than the spell's own parameter key, so type-checking
     * it here would stop a saved spell parameter from restoring. An empty slot
     * also passes - there is nothing to seed with, so the seed is "" either
     * way, and a false block would only risk breaking a legitimate restore. The
     * two TARGET families (Cidade*, Coordenada*) match as a family rather than
     * string-for-string, so swapping one hex order for another still carries
     * the target across even when the range differs: those widgets only ever
     * select from their own model, and a value they do not hold falls back to
     * the actor's hex, so a range-5 picker cannot keep a range-12 target. Same
     * two prefixes the blank-target save guard uses. Static and public so the
     * rule can be pinned by a test without standing up the panel.
     */
    ...
    /**
     * True when both controls are hex targets, or both are city targets (see
     * {@link #isSameParametroTipo}).
     */
