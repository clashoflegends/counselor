...
import gui.services.LoyaltyDeltaTableCellRenderer;
...
//    private SubTabConfigActor stRename = new SubTabConfigActor();
    ...
    private LoyaltyDeltaTableCellRenderer ldcr;
    ...
    private void initConfig() {
        ...
        //loyalty-var column: diverging scale - red/amber for drops, green/blue for gains.
        ldcr = new LoyaltyDeltaTableCellRenderer();
    ...
    public final void setMainModel(TableModel model) {
        ...
        if (ldcr != null) {
            jtMainLista.getColumnModel().getColumn(jtMainLista.getColumn(labels.getString("LEALDADE.VARIACAO")).getModelIndex()).setCellRenderer(ldcr);
        }
        this.updateGui();
        this.doTagHide();
    ...
    /** {@inheritDoc} A city's panel text: its order outcomes plus its production/status detail. */
    @Override
    public String getIndexableText(int modelRow) {
        final Object city = actorAt(this.cidadeControl == null ? null : this.cidadeControl.getListaExibida(), modelRow);
        return (city instanceof model.Cidade) ? control.services.CidadeConverter.getResultados((model.Cidade) city) : "";
    }
}
