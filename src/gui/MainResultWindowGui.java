...
public class MainResultWindowGui extends javax.swing.JPanel implements Serializable {
    ...
    public MainResultWindowGui(String autoLoad) {
        ...
        jbCopyTeam.addActionListener(wc);
    ...
    /**
     * Load a crisp, theme-aware toolbar icon from the bundled Tabler (MIT) SVG
     * set. The monochrome stroke icon is painted in the current theme's button
     * text colour, so it adapts light/dark (icons are built once at startup;
     * the theme is fixed per session).
     */
    ...
    /**
     * Replace the Matisse-generated raster toolbar icons with vector SVG icons
     * (called after initComponents so it doesn't fight the .form). Score-graph
     * buttons keep the bar/area/pie split.
     */
    private void applyToolbarIcons() {
        ...
        // jbCopyTeam: Matisse cloned it from jbCopy (same labels + actionCommand "jbCopy"); override here
        // so it dispatches to its own handler and reads its own team-specific labels/icon.
        jbCopyTeam.setIcon(svgIcon("users"));
        jbCopyTeam.setActionCommand("jbCopyTeam");
        jbCopyTeam.setText(labels.getString("COPIAR.ACOES.TEAM"));
        jbCopyTeam.setToolTipText(labels.getString("COPIAR.ACOES.TEAM.TOOLTIP"));
    ...
    /**
     * Build and show the recently-opened-files popup below the Recent toolbar
     * button.
     */
    ...
    private void initComponents() {
        ...
        jbCopyTeam = new javax.swing.JButton();
        ...
        jbCopyTeam.setText(labels.getString("COPIAR.ACOES")); // NOI18N
        jbCopyTeam.setToolTipText(labels.getString("COPIAR.ACOES.TOOLTIP")); // NOI18N
        jbCopyTeam.setActionCommand("jbCopyTeam");
        jbCopyTeam.setEnabled(false);
        jbCopyTeam.setFocusable(false);
        jbCopyTeam.setName("jbCopy"); // NOI18N
        jToolBar1.add(jbCopyTeam);
    ...
    private javax.swing.JButton jbCopyTeam;
    ...
    /**
     * The currently open game file name (e.g. "game_884_13.gurgel.rr.egf"), or
     * "" if none.
     */
    ...
    /**
     * Flags that a newer Counselor release is available (version like "2.865");
     * shown in title + status bar.
     */
    ...
    /**
     * Re-apply the window title once this panel is attached to its frame.
     * Autoload (file-association / properties.config) opens the EGF inside the
     * constructor, before PbmApplication calls setContentPane()/setVisible(),
     * so refreshWindowTitle() there finds no frame ancestor and no-ops.
     * addNotify() fires during frame.pack(), when getWindowAncestor() resolves,
     * so the stored openFileName is applied then.
     */
    @Override
    ...
    /**
     * Re-apply the window title after the frame is fully realized and visible.
     * addNotify() during frame.pack() proved unreliable for the autoload /
     * file-association (double-click) path: the title set re-entrantly while
     * the frame's native peer is still being created did not stick, so a
     * double-clicked EGF stayed "Counselor" instead of "Counselor - filename".
     * PbmApplication calls this once after frame.setVisible(true), where the
     * ancestor resolves and nothing re-titles after.
     */
    ...
    /**
     * Set the info-bar background and every label's foreground together so the
     * bar stays readable on any theme. urgent=true for the deadline-due /
     * game-over warning colors.
     */
    ...
    /**
     * Order-sync indicator (loaded orders vs. what the server holds). The
     * controller drives this on load / edit / send. Rendered on the far-right
     * status-bar label: a state-coloured dot plus the localized state word (the
     * text follows the theme foreground, so it stays legible white-on-dark /
    ...
    private void renderOrderSync(control.services.OrdersHashService.OrderSyncState state) {
        ...
        switch (state) {
            case SENT:
                key = "ORDERSYNC.SENT";
                dot = new Color(0x2E9E44);
                break; // green
            case PENDING:
                key = "ORDERSYNC.PENDING";
                dot = new Color(0xD9920A);
                break; // amber
            case NO_TOKEN:
                key = "ORDERSYNC.NOTOKEN";
                dot = new Color(0xC62828);
                break; // red
            default:
                key = "ORDERSYNC.UNKNOWN";
                dot = new Color(0x9E9E9E);
                break; // grey
        }
    ...
    /**
     * Blink jbSend blue&lt;-&gt;grey using the ready toast's exact palette +
     * cadence (Toast.FLASH_*), so the button and the toast pulse together, then
     * settle on the current order-sync colour. Called by the controller at the
     * "orders ready" moment (once per turn).
     */
    ...
    /**
     * Stop any running submit blink (e.g. on turn reset) and settle the icon to
     * the at-rest colour.
     */
    ...
    /**
     * A small filled status dot with a darker ring, so it reads on either a
     * light or dark status bar.
     */
    ...
    /**
     * Army-movement-path overlay toggle state, persisted by doDrawArmyPaths as
     * "drawArmyMovPath" (1/0; default on). Was hardcoded to true at init, so
     * the saved choice was ignored on restart.
     */
