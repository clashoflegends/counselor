...
**Privacy policy:** Counselor communicates only with the game's own server at `clashlegends.com` and uses no third-party analytics, advertising, or tracking. When you submit orders it sends your turn file and orders, your player login and upload token, the game/turn identifiers, and basic client/environment details (versions, OS, screen size, install type, startup time, UI settings) used to support the app; on a crash it may send a crash report (error, stack trace, login, current game); portrait packs and update checks are fetched from the same server when you request them. Full policy: [PRIVACY.md](PRIVACY.md).
